...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (Number.isNaN(filter.price?.$gte) || Number.isNaN(filter.price?.$lte) || (filter.price?.$gte !== undefined && filter.price?.$lte !== undefined && filter.price.$gte > filter.price.$lte)) return res.status(400).json({ message: 'Provide a valid price range.' });
    let products = await Product.find(filter).populate({ path: 'entrepreneur', match: { verificationStatus: 'approved', ...(location && { location: { $regex: location.trim(), $options: 'i' } }) }, populate: [{ path: 'user', select: 'name' }] }).populate('category', 'name').sort('-createdAt');
...
router.get('/:id', async (req, res, next) => { try { const product = await Product.findOne({ _id: req.params.id, isAvailable: true, stock: { $gt: 0 } }).populate({ path: 'entrepreneur', match: { verificationStatus: 'approved' }, populate: { path: 'user', select: 'name' } }).populate('category', 'name'); if (!product?.entrepreneur) return res.status(404).json({ message: 'Product not found or unavailable.' }); res.json(product); } catch (error) { next(error); } });
